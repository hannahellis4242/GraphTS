...
describe("Undirected Graph", () => {
  test("Graph starts off empty", () => {
    const graph = new UndirectedGraph<null, string, number>(null);
  ...
  test("Adding an edge between two vertices", () => {
    ...
    expect(inEdges(a, graph)).toHaveLength(1);
    {
  ...
  test("Adding two edges", () => {
    ...
    expect(outEdges(a, graph)).toHaveLength(1);
    expect(inEdges(a, graph)).toHaveLength(1);
    expect(outEdges(b, graph)).toHaveLength(2);
    expect(inEdges(b, graph)).toHaveLength(2);
    expect(outEdges(c, graph)).toHaveLength(1);
    expect(inEdges(c, graph)).toHaveLength(1);
    {
      const result = adjacentVertices(a, graph);
      expect(result).toHaveLength(1);
      expect(result).toContain(b);
